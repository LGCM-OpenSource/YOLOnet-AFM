...
df_path = f'models{os.sep}training_upp_init_filter.log'
...
for title, dfs in list_df_validation_metrics:
    fig = chart.line_plot(dfs, x='epoch', y='scores', color = 'metrics', title=f'Unet_pp_6_channels_initFilter_64_training_validation: {title}')


    fig.write_image(f'Unet_pp_6_channels_initFilter_64_training_validation_{title}.png')
