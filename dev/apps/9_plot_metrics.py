...
final_df_melt_general = pd.melt(final_df_general_validation, id_vars=['Process Date','Model'], value_vars=['Precision', 'Recall', 'F1', 'Dice'], var_name = 'metrics', value_name = 'scores')    
...
fig = chart.box_plot(final_df_melt_general, x='Model', y='scores', color = 'metrics', title='Unet_6_channels_on_Test_set')
fig2 = chart.box_plot(final_df_melt, x='Model', y='scores', color = 'metrics', title='Unet_6_channels_on_Test_set_per_fold')

# fig.show()
fig.write_image(f'unet_AFM_6_channels_all.png')
