# /home/arthur/lgcm/projects/Unet_AFM/dev/apps/3_predicts.py
import os
import sys
from utils import UnetProcess, Models, build_file_path, CROP_PATH, UNET_MODELS_PATH, TerminalStyles, setup_logger
from tqdm import tqdm
import argparse
import traceback # Import traceback for better error logging if needed

logger = setup_logger('Model predicts')

def build_paths(files_list, actual_process='_channels_added.npy'):
    """Builds and validates paths for processing."""
    valid_data = []
    logger.info(f"Building paths for {len(files_list)} potential files...")
    ...
    for img_file in files_list:
        ...
        pre = build_file_path(model_info['preprocess_img'], img_file) # Use the original file name from listdir
        mask = build_file_path(model_info['preprocess_mask'], img_file) # Use the original file name from listdir
        save = build_file_path(model_info['save_predict'], base_name + '_unet.png') # Construct save path from base name
        ...
        if missing:
            logger.warning(f"File '{img_file}' related paths ignored. Missing: {', '.join(missing)}")
            logger.debug(f"  - Optical expected: {opt}")
            logger.debug(f"  - Usefull expected: {use}")
            logger.debug(f"  - Preprocess expected: {pre}")
            logger.debug(f"  - Mask expected: {mask}")
        else:
            valid_data.append((opt, use, pre, mask, save))

    if not valid_data:
        logger.error("No valid data found after checking paths.")
        return [], [], [], [], []

    logger.info(f"Found {len(valid_data)} valid sets of files.")
...
if __name__ == "__main__":
    logger.info(f"Starting prediction process for model: {model_selector}")
    logger.info(f"Visualization enabled: {visualize_segmentation}")
    ...
    try:
        ...
        if not os.path.isdir(model_info['preprocess_img']):
             logger.error(f"Preprocess image directory not found: {model_info['preprocess_img']}")
             sys.exit(1)

        files_list = os.listdir(model_info['preprocess_img'])
        if not files_list:
            logger.error(f"No files found in preprocess image directory: {model_info['preprocess_img']}")
            sys.exit(1)
        ...
        if not opt_image_path:
            logger.error("No valid file sets found to process. Exiting.")
            sys.exit(1)

        # Load the model
        logger.info(f"Loading model: {model_info['model_name']} from {model_info['model_path']}")
        ...
        logger.info(f"Processing {len(opt_image_path)} images...")
        for i in tqdm(range(len(opt_image_path)), desc="Predicting", colour='#0000FF'):
            try:
                ...
                logger.error(f"File not found during processing loop for index {i}: {fnf_error}")
            except Exception as loop_error:
                logger.error(f"Unexpected error during processing loop for index {i} ({os.path.basename(opt_image_path[i])}):", exc_info=True)
        ...
        logger.error(f"Invalid model selection key: '{model_selector}'. Check UNET_MODELS_PATH in data_path.py.")
        sys.exit(1)
    except Exception as main_error:
        logger.critical("An unexpected error occurred in the main prediction script:", exc_info=True)
        sys.exit(1)